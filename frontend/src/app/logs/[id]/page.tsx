  ...
  useEffect(() => {
    (async () => {
      const response = await fetch(
        `http://localhost:1337/getContainerLogs?containerId=${containerId}`,
        {
          method: "GET",
          headers: {
      ...
      <DefaultLayout>
        <div className="flex w-full flex-row">
          {errorDetails ? (
            <>
              <div className="w-2/3">
                <div className="m-5 text-lg font-bold text-white">
                  Error details
                </div>
                <div>
                  <pre className="my-5 whitespace-pre-wrap text-wrap  p-5 text-xl text-white">
                    {currentError.title}
                  </pre>
                </div>
                ...
                <div>
                  {loading ? (
                    <div className="m-5 whitespace-pre-wrap  text-wrap p-5 text-white">
                      Getting response from Dock Ai...
                    </div>
                  ) : (
                    //ignore the error
                    // @ts-ignore
                    <div className="whitespace-pre-wrap text-wrap p-5">
            ...
            <div className="w-2/3">
              ...
              <div>
                <pre className="m-5 whitespace-pre-wrap  text-wrap p-5 text-white">
                  {logs}
                  {/* {demoLogs} */}
                </pre>
              </div>
          ...
          <div className="w-1/3">
            <div className="m-5 text-lg font-bold text-white">Errors</div>
            <div>
              <div className="rounded-sm border border-stroke bg-white p-4 shadow-default dark:border-strokedark dark:bg-boxdark md:p-6 xl:p-9">
                <div className="flex flex-col gap-7.5">
                  {errorList.map((error: CustomError) => (
                    <div
                      className="flex w-full cursor-pointer border border-l-6 border-[#F87171] bg-[#F87171] bg-opacity-[15%] p-3 shadow-md hover:border-red dark:bg-[#1B1B24] dark:bg-opacity-30  md:p-3"
                      onClick={() => {
